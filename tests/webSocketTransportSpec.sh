...
rg -Fq 'bool begin(const char* devicePid, const char* deviceSecret);' "$HEADER_PATH"
rg -Fq 'return begin(config);' "$ROOT_DIR/src/Atanstack.cpp"
rg -Fq 'AtanstackEvent event(const char* eventType);' "$HEADER_PATH"
rg -Fq 'AtanstackEvent& data(const char* key, const T& value);' "$HEADER_PATH"
rg -Fq 'AtanstackEvent& meta(const char* key, const T& value);' "$HEADER_PATH"
rg -Fq '_client->setError("missing_data");' "$ROOT_DIR/src/Atanstack.cpp"
rg -Fq 'setError("missing_device_pid");' "$ROOT_DIR/src/Atanstack.cpp"
rg -Fq 'setError("missing_device_secret");' "$ROOT_DIR/src/Atanstack.cpp"
...
if rg -q 'WiFiClientSecure|MQTT_HOST|MQTT_PORT|MQTT_PATH|config\.(brokerHost|brokerPort|webSocketPath)' "$SKETCH_PATH"; then
  echo "cloud sketch overrides AtanStack Cloud defaults" >&2
  exit 1
fi
